...
export class PizzaComponent implements OnInit {
  ...
  listenToCartUpdates() {
    this.pizzaService.cartUpdated.subscribe((orderItems) => {
      const cartItem = orderItems.find(
        (i) => i.id === this.pizza.id && i.size === this.selectedSize.size
      );
  ...
  addToCart() {
    ...
    const selectedPizza: OrderItem = {
      ...
      itemValue: this.selectedSize.price,
      ingredients: [],
    };
  ...
  updateCartItems() {
    ...
    if (cartItem) {
      ...
      cartItem.itemValue = this.selectedSize.price * this.selectedQuantity;
      this.pizzaService.orderItems = [...this.pizzaService.orderItems];
    }
  }
